...
Every byte of tool output joins context permanently. The first priority is producing less output, not managing large output after the fact.
...
If sub-agents are available, delegate tool calls through them as a context blast shield. Sub-agents should also batch and pipe internally using the patterns above. See `references/sub-agent-delegation.md`.
